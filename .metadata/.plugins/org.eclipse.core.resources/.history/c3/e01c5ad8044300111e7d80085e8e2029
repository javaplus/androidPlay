...
public class World {
	...
	public void update(float deltaTime){
		...
		while(tickTime > tick){
			...
			// check if stain was hit by head
			if(head.x == stain.x && head.y == stain.y){
				score += SCORE_INCREMENT;
				snake.eat(stain.type);
				...
				// increase speed of snake after eating so many stains
				if(score %100 ==0 && tick - TICK_DECREMENTAL > 0){
					tick -=TICK_DECREMENTAL;
				}
			}
		}
	}
